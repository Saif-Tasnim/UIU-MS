...
const Blog = () => {
    ...
    const { data: blogs = [], refetch } = useQuery({
        queryKey: ['blogs'],
        queryFn: async () => {
            const res = await axiosSecure.get('/postBlog');
            return res.data;
        }
    });
    ...
    //     // console.log(id);
    //     const res = await axiosSecure.patch(`/postBlog/${id}`)
                            ...
                            blogs.map((b, index) =>
                                <article key={index} className="flex flex-col bg-slate-100 rounded-xl p-3">
                                    <div className="flex flex-col p-6 ">
                                        ...
                                        <div className='flex justify-between items-center text-sm my-7'>
                                            <div className='flex flex-col gap-y-4'>
                                                <h1 className='text-blue-600'> {b.firstName} {b.lastName} </h1>
                                                <p className='text-blue-600'> {b.email} </p>
                                            </div>
